...
function countYearsInOffice(presidentsArr) {
  let accumulator = 0;
  const yearsInOffice = presidentsArr.reduce((acc, onePres) => {
    if (!onePres.leftOffice) {
      return acc;
    }
    const yearsOfOnePres = onePres.leftOffice - onePres.tookOffice;
    return acc + yearsOfOnePres;
  }, 0);
...
function sortPresidentsByBirthYear(presidentsArr) {
  ...
  // });
  presidentsArr.sort(
    (currentPres, nextPres) => currentPres.birthYear - nextPres.birthYear,
  );
